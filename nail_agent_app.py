...
_dna_addon = ""
try:
    with open("store_dna.json", encoding="utf-8") as _f:
...
# ─── Session State ─────────────────────────────────────────────────────────────
os.makedirs("uploads", exist_ok=True)   # 商家上传图片目录
...
_df_base = load_data("mock_data.csv")
...
with st.sidebar:
    ...
    if _show_truth:
        try:
            with open("mock_data_truth.json", encoding="utf-8") as _f:
                _truth = json.load(_f)
...
def _load_content_pool() -> list:
    try:
        with open("content_pool.json", encoding="utf-8") as _f:
            return json.load(_f)
...
def _save_to_content_pool(new_entries: list):
    ...
    with open("content_pool.json", "w", encoding="utf-8") as _f:
...
with tabs[0]:
    ...
    try:
        _ugc_p = load_ugc_ext("ugc_external.csv")
        _ud    = _ugc_p["日期"].max()
...
with tabs[6]:
    ...
    try:
        _ue = load_ugc_ext("ugc_external.csv")
        _ui = load_ugc_int("ugc_internal.json")
    except FileNotFoundError:
                    ...
                    with st.spinner("甲心助手正在识别美甲图…"):
                        try:
                            ...
                            for _i2, (_uf2, _tg) in enumerate(zip(_up_files, _tags_list)):
                                _uf2.seek(0)
                                _sp = f"uploads/{_now_str[:10]}_{_i2}_{_uf2.name}"
    ...
    _dna_d = None
    try:
        with open("store_dna.json", encoding="utf-8") as _ff:
